...
async def enter_test(message: Message):
    await message.answer(
        ...
        'Вопрос №1 \n\n'
        'Вы часто занимаетесь бессмысленными делами (бесцельно'
        'блуждаете по интернету, клацаете пультом телевизора, просто'
        'смотрите в потолок)?'
    )
...
async def answer_q1(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        # Или data['some_list'].append(1), т.к. не нужно сначала
        # доставать из стейта, а потом задавать.

    await message.answer(
        'Вопрос №2. \n\n'
        'Ваша память ухудшилась и вы помните то, что было давно, но'
